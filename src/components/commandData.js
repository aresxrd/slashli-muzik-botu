...
async function setVolume(interaction) {
  ...
  if (volume < 1) return interaction.followUp("Ses Seviyesi Minimum 1 Olabilir.");
  if (volume > 100) return interaction.followUp("Ses Seviyesi Maximum 100 Olabilir.");

  distube.setVolume(guild, volume);
  return interaction.followUp(`Müziğin ses oranı başarıyla **${volume}** olarak ayarlandı!`);
}
...
async function setSpeed(interaction) {
  ...
  const speedMessage = speed
    ? "Şarkı başarıyla eski hızına döndürüldü."
    : "Şarkı başarıyla hızlandırıldı.";
...
async function setBass(interaction) {
  ...
  const bassMessage = bass
    ? "Şarkı başarıyla eski haline döndürüldü."
    : "Şarkı başarıyla basboosted yapıldı.";
